...
{
    public class CriarContatoHandler : IRequestHandler<CriarContatoRequest<HandlerResponse<Contato>>, HandlerResponse<Contato>>
    {
        private IViaCepRepository ViaCepRepository { get; set; }
        private IContatosRepository ContatosRepository { get; set; }

        public CriarContatoHandler(IContatosRepository contatosRepository, IViaCepRepository viaCepRepository)
        {
            ContatosRepository = contatosRepository;
            ViaCepRepository = viaCepRepository;
        }

        public async Task<HandlerResponse<Contato>> Handle(CriarContatoRequest<HandlerResponse<Contato>> request, CancellationToken cancellationToken)
        {
            request.Validar();
            ...
            var contato = await ContatosRepository.Inserir(request.Contato);
